...
(feito no editor de .md, deve não funcionar...)
...
```c
   ...
   int main()
   {
      ...
      int ds = 20; // base dos dados
      int progr[] = { 2, 0, 7, 2, 10, 5, ds,    //      x=0; l=10
                      8, 20, 1,                 // ali: print x
                      9, 8, 11, ds, 18, 7,      //      if ++x != l goto ali
                      1                         //      FIM
                    };
                    
      // variáveis que representam o computador
      mem_t *mem = mem_cria(ds+1);
      es_t *es = es_cria();
      cpu_t *cpu = cpu_cria();
      cpu_estado_t *estado = cpu_estado_cria();
      
      // copia o programa para a memória
      for (int i=0; mem_tam(mem); i++) {
        mem_escreve(mem, i, progr[i]);
      }
      
      // iniacializa a CPU com as variáveis criadas
